...
static void set_ble_status_on_ui(void *arg) {
    eink_ui_set_ble_status((bool)(intptr_t)arg);
}

static void post_ble_status(bool connected) {
    if (!platform_task_post_to_ui(set_ble_status_on_ui,
                                  (void *)(intptr_t)connected)) {
        ESP_LOGW(TAG, "UI queue full; BLE status update dropped");
    }
}

static void post_consumer_action(controller_input_action_t action) {
    if (!controller_input_post_action(action)) {
        ESP_LOGW(TAG, "UI queue full; BLE media action %d dropped", action);
    }
}
...
static void handle_consumer_control(const uint8_t *data, uint16_t len) {
    ...
    case 0x00CD:  // Play/Pause
        post_consumer_action(CONTROLLER_INPUT_PLAY_PAUSE);
        break;
    case 0x00B5:  // Next Track
        post_consumer_action(CONTROLLER_INPUT_NEXT_TRACK);
        break;
    case 0x00B6:  // Previous Track
        post_consumer_action(CONTROLLER_INPUT_PREV_TRACK);
        break;
    case 0x00E9:  // Volume Up
        post_consumer_action(CONTROLLER_INPUT_VOL_UP);
        break;
    case 0x00EA:  // Volume Down
        post_consumer_action(CONTROLLER_INPUT_VOL_DOWN);
        break;
...
bool ble_remote_init(void) {
    ...
    if (!s_mutex || !s_scan_sem) {
        ESP_LOGE(TAG, "Could not allocate BLE synchronization primitives");
        if (s_mutex) {
    ...
    if (s_has_bonded) {
        if (xTaskCreate(reconnect_task, "ble_reconn", 4096,
                        NULL, 2, NULL) != pdPASS) {
            ESP_LOGE(TAG, "Failed to create BLE reconnect task");
        }
    }
...
void ble_remote_scan_start(void) {
    if (!s_initialized) {
        ESP_LOGW(TAG, "BLE remote is unavailable");
        return;
    }
    if (s_scanning) {
...
void ble_remote_pair(int index) {
    ...
    if (xTaskCreate(pair_task, "ble_pair", 4096,
                    (void *)(intptr_t)index, 3, NULL) != pdPASS) {
        ESP_LOGE(TAG, "Failed to create BLE pairing task");
    }
}
