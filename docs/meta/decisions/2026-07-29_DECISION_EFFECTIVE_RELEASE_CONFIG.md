...
The runtime-adjustment evidence therefore stands on `aee0a78` itself rather than being transferred from an earlier tree: the job graph, `build-idf` host assertion, 83-case suite and `--forbid-absent-satisfied` implementation all ran in container CI. `a9e6fd3` then reproduced them from a byte-identical executable tree. The preview edge can increase latency because the dependent waits for the maximum of three jobs, but it added **zero observed wall-clock wait** in both recorded schedules: `build-stale-config` finished before `build-idf`, which was already the critical path. The cost is potential variance and failure coupling, not a measured delay in these two runs. No run is claimed for the documentation-only commit carrying this paragraph; none is needed to support a new behaviour claim, because it changes no executable or configuration input.
