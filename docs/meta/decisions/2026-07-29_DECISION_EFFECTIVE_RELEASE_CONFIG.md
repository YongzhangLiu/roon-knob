...
**Scope of that observation, stated precisely.** Run `30506101326` executed the tree at `62ffc4a`, which is the **parent** of the commit carrying this paragraph. Every code, workflow, CMake, checker and fixture file is byte-identical between the two — the successor is documentation only — so the evidence above describes this tree's behaviour. It will nevertheless be re-run by CI on the docs-only successor, and a *material* divergence between the two runs would itself be a finding worth investigating rather than explaining away.
