...
**Not on this list, and checked rather than assumed:** the deltas to `idf_app/tools/assert_release_report.py` (its docstring, rescoping "CI cannot opt out" to the narrower claim that actually holds) and to `idf_app/tools/check_release_config.py` are **comment-only** in the same mechanical sense used above, as is `idf_app/sdkconfig.defaults`. None of the three changes a byte the interpreter executes, so none of them qualifies a run's evidence, and counting them here would have inflated the surface in the opposite direction from the undercount being corrected.

Deliberately **no run is claimed for this commit**. A record cannot cite CI for the tree that contains the citation — the run would have to exist before the commit that references it — and inventing a self-referential entry is the failure mode this section is written to avoid. That principle is unchanged.
...
The diff from `1eae955bb4aba60673396eae91b7cf2903fa99f6` to this commit touches **four** files, and an earlier revision of this paragraph named only three — it omitted `idf_app/sdkconfig.defaults`, which is the one of the four a reader is least likely to assume is inert. All four, and what each is:

* `docs/dev/KCONFIG.md` — documentation.
* this record — documentation.
* `idf_app/tools/check_release_config.py` — **comment-only**: the `STRING_INVARIANTS` comment above `ESPTOOLPY_FLASHSIZE` is rewritten from the "takes no position" framing to the pinned-value-with-deferred-direction stance. The `STRING_INVARIANTS` tuple itself, and every other executable line, is unchanged.
* `idf_app/sdkconfig.defaults` — **also comment-only**: the explanatory block above the optimization line is rewritten to name `tools/check_release_config.py` as the file that owns the accepted set. The assignment `CONFIG_COMPILER_OPTIMIZATION_SIZE=y`, and every other assignment in the file, is unchanged. This matters more than an ordinary comment edit, because `sdkconfig.defaults` is an *input to the build*: had a line here moved, the resolved configuration and therefore `config_digest` would have moved with it, and the carry-forward argument below would be void.

Both code deltas are **mechanically comment-only** — verified by filtering each file's diff to non-comment, non-blank lines, which is empty in both cases. So every checker, gate, workflow, CMake, fixture and defaults byte that run `30512450762` executed is the byte in this tree, and the byte-identity argument applies exactly as it did when it carried run `30506101326`'s evidence to `876c0786…`.

The honest position is therefore stronger than it was, and still bounded the same way: the container-CI evidence stands for its own exact SHAs; the nearest of those SHAs is behaviourally identical to this tree; the message-attribution path is covered both by the committed suites here (77/77 and 23/23, the latter passing under a quiet interpreter and a noisy one) and by the before/after headline ordering observed in the container; and a run at *this* commit is still not claimed, because it does not exist yet. If one is produced and diverges from the tables, that remains a finding.
...
* **What is true:** *within a workflow run*, a `build-idf` job cannot run **unenforced**. Enforcement is forced on the `idf.py` command line and then re-derived from the report on the host, so a flipped flag, a deleted checker or a removed CMake call fails the job. And **publication** is genuinely gated by a real in-repo dependency, not a convention — but the two consumers gate on *different* sets, and an earlier revision of this record blurred them:
  ...
  * `deploy-pr-preview` needs **only two**: `needs: [build-idf, release-config-fixtures]`. So a red `build-stale-config` **alone does not stop a flashable preview** — a preview can be published from a tree whose stale-config integration proof is failing, as long as the build and the fixtures are green. This is a deliberate cost/coverage trade (the preview path does not pay for the container negative), but it is a gap in the gating claim and is recorded here as one rather than left implied. Whether previews should require that proof is now tracked as [**#207**](https://github.com/muness/roon-knob/issues/207) (*Decide whether PR previews require the stale-config blocking proof*) — an explicit trust contract for publicly flashable previews, decided rather than inherited from this trade.
...
| [#206](https://github.com/muness/roon-knob/issues/206) | Fail closed when negative release invariants disappear | "Absence is directional" — an expected-`false` symbol that leaves the SDK schema reads as satisfied | coverage gap whose failure mode is **green** |
| [#207](https://github.com/muness/roon-knob/issues/207) | Decide whether PR previews require the stale-config blocking proof | "Enforcement ceilings" / "Negative proof" — `deploy-pr-preview` needs two of the three proof jobs | explicit trust contract for a deliberate trade |
...
Three properties they share, worth stating so none of them is later read as a licence to loosen the gate:
...
* **#206 and #209 are one model, two risks.** Both need active Kconfig/schema evidence rather than an inference from absence in `sdkconfig.json` — which is why neither is a small patch to this checker — and they should land with one documented semantics for symbol disappearance without conflating a shrinking-coverage risk (#206) with a misattribution risk (#209). Both coordinate with **#183**, since an SDK pin/bump is when symbol disappearance actually happens.
* **#205 and #207 are decisions, not code.** One is a repository setting requiring maintainer authorization; the other is a choice about what a publicly flashable preview is allowed to mean. Neither is blocked on implementation, and neither is #202's to make.

These are distinct from the **#203 handoff above**, which carries #202's *consequences* (geometry, size headroom, provenance, the hardware half). The five here are gaps in #202's own enforcement story, disclosed by this record and deliberately left open by it.
...
* All four runs are attributed to the exact commits they executed; see "How to read all four runs" in the Status block for what they do and do not cover.
* Tracked follow-ups from this record: [#205](https://github.com/muness/roon-knob/issues/205), [#206](https://github.com/muness/roon-knob/issues/206), [#207](https://github.com/muness/roon-knob/issues/207), [#208](https://github.com/muness/roon-knob/issues/208), [#209](https://github.com/muness/roon-knob/issues/209) — see "Tracked follow-ups". Adjacent: **#203** (geometry, size headroom, provenance, hardware), **#183** (SDK pinning), **#193** (hardware identity), **#200** (config profiles).
