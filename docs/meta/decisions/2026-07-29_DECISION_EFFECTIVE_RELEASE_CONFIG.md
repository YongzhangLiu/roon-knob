...
* `idf_app/tools/test_check_release_config.sh` — **executable**: six cases covering the new flag positively and negatively, its vacuous-pass guard, and a malformed-row guard. The host suite is **83 cases in this tree**, where all four recorded runs observed 77. As with the CMake suite's `13/13` and `17/17`, the `77/77` figures in the tables above are left as recorded: they are what those runs actually reported at those SHAs.
...
So the bounded position, restated for what this tree actually is: the container-CI evidence stands for its own exact SHAs and is **not** transferred to this tree by byte identity, because this tree is no longer byte-identical. What does carry is narrower and stated as such — the config-resolution half, because the checker and the defaults are untouched. What does **not** carry is the job graph and `build-idf`'s host-assertion step, both of which changed and neither of which any recorded run exercised in its present form; in particular `deploy-pr-preview`'s recorded `0m07s` was measured when it waited on two jobs, not three, so expect it to start later now (it adds waiting, not a build, since `build-stale-config` already runs in every triggered workflow, including every pull-request run). The two adjustments are covered by the committed host suite at 83/83, which is host evidence, not container evidence, and the record does not promote one to the other. A run at *this* commit is still not claimed, because it does not exist yet. When one is produced, the figures to check are `83/83` host, `23/23` CMake, a green `deploy-pr-preview` gated on all three proof jobs, and `RK-RELCFG-ASSERT-OK: enforced=true verdict=pass digest-matched no-absent-satisfied` from `build-idf`. Divergence from any of those is a finding.
...
These are distinct from the **#203 handoff above**, which carries #202's *consequences* (geometry, size headroom, provenance, the hardware half). Four remain open gaps in #202's enforcement story; #207 is retained in the table as the decision that closed the fifth.
