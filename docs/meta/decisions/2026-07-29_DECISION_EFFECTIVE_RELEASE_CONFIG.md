...
**Observed:** the host half. `tools/test_check_release_config.sh` passes (exit contract, token precedence, absence-directionality, secret non-leakage, assert-the-assertion, caller-owned marker, empty-argument contract); the CMake gate's decision logic, including its `;`-sanitisation, non-empty first line, fail-closed-on-missing-checker and fail-closed-on-unwritable-report paths, is exercised by a stubbed `cmake -P` harness; every host `run:` step in `build-idf`, `build-stale-config` and `release` has been executed against a simulated runner (including a root-owned read-only `build/config`, a missing report, a wrong log, and differing ON/OFF digests).
...
Two containers rather than one, so each outcome is separately attributable in the log; run 2 is incremental (enforcement is not a compile flag, so objects are reused). Neither run calls `set-target`, which would rename the seeded `sdkconfig` to `sdkconfig.old` and destroy the condition under test.
...
* `idf_app/build/config/rk_release_config.json` rides along in the existing `esp32s3-firmware` artifact, next to the binary it describes.
