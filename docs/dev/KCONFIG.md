...
- **CI cannot use it, and cannot silently build unenforced.** The workflow passes `-DRK_ENFORCE_RELEASE_CONFIG=ON` explicitly and then asserts the report on the host, so a `build-idf` job cannot run with enforcement off, and an artifact built with enforcement off cannot be **published**. Both publishing consumers gate eligibility on the **same three** proof jobs — `release` and `deploy-pr-preview` each declare `needs: [build-idf, release-config-fixtures, build-stale-config]` — so any one of them red or skipped stops both a `v*` tag from publishing firmware and a PR preview from being flashed. This is prerequisite parity, not complete artifact-validation parity: `release` re-asserts the downloaded report before publishing, while `deploy-pr-preview` does not yet; #210 tracks that caller-workflow gap. (An earlier version of this page described `deploy-pr-preview` as needing only two of the three; that prerequisite asymmetry has been closed.)
